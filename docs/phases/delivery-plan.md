...
**最关键的是 Phase 3：真实 BLE 小车闭环。** 软件不再生成本地造数，必须以真实 ESP32 小车上报为准。
...
## Phase 3：真实 BLE 小车闭环（最关键）

### 目标

在 ESP32 小车在线的情况下，跑通"上位机 → App → BLE 小车 → 上位机"完整闭环。

### 交付物

- CarTransport 抽象接口
- BLE notify 遥测接收
- 真实 trace 执行状态上报（progress 0→100）
- BLE move / lift / stop / estop 指令下发
- carStore 状态管理
...
- [ ] 每个上位机命令都能看到完整状态链：
  - 已创建 → 已发送 → App 已接收 → BLE 小车已执行 → 完成/失败
- [ ] 超时命令显示 timeout
- [ ] command_logs 表有完整记录
